...
export default function Confirmation(props) {
    ...
    useEffect(()=>{
        const passengerListClone = _.cloneDeep(tour_context?.userInputData?.passengerList);
        if (passengerListClone?.length == 1) {
            setBillingPerson(tour_context?.userInputData?.passengerList)
        } else {
    ...
    return (
        <div style={{ display: `${props.progressIndex == 99 ? "block" : "none"}` }}>
            ...
            <p className="d-flex summary-desc justify-content-center align-self-center text-center mb-3 mb-xxxl-0" style={{fontFamily: '"Poppins"', fontWeight: 400, fontSize: '18px', maxWidth: '600px'}}>
                Payment is successful and you should receive an email through {tour_context?.userInputData?.email} for the full itenerary and details of the booking.
            </p>
