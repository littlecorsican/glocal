...
export default function Confirmation(props) {
    ...
    useEffect(()=>{
        const passengerListClone = _.cloneDeep(umrah_context?.userInputData?.passengerList);
        if (passengerListClone?.length == 1) {
            setBillingPerson(umrah_context?.userInputData?.passengerList)
        } else {
    ...
    },[umrah_context?.userInputData])

    useEffect(()=>{
        console.log("billingPerson", billingPerson)
    },[billingPerson])
    ...
    return (
        <div style={{ display: `${props.progressIndex == 99 ? "block" : "none"}` }}>
            ...
            <p className="d-flex summary-desc justify-content-center align-self-center text-center mb-3 mb-xxxl-0" style={{fontFamily: '"Poppins"', fontWeight: 400, fontSize: '18px', maxWidth: '600px'}}>
                Payment is successful and you should receive an email through {umrah_context?.userInputData?.email} for the full itenerary and details of the booking.
            </p>
                ...
                <div className="table_div" style={{backgroundColor: '#f5f5f5', borderRadius: '10px'}}>
                    <table className="m-0 w-100">
                        ...
                        <tr>
                            ...
                            <td style={{fontFamily: '"Poppins"', fontWeight: 600, fontSize: '16px'}} className="col-xl-9 col-5 p-3 fw-bold">{umrah_context?.userInputData?.passengerList.length || 0}</td>
                        </tr>
                        </tbody></table>
                    </div>
                    ...
                    <table className="m-0 w-100 overflow-scroll">
                        ...
                        <tr style={{borderBottom: '1px solid #e8e2d6'}}>
                            <th style={{fontFamily: '"Poppins"', fontWeight: '400!important', fontSize: '16px'}} scope="row" className="col-xl-4 col-5 p-3">Passport Number</th>
                            <td style={{fontFamily: '"Poppins"', fontWeight: 600, fontSize: '16px'}} className="col-xl-9 col-5 p-3 fw-bold">
