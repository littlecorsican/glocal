...
export default function Cruise() {
    ...
    useEffect(() => {
        ...
        setLoadingText("Loading...")
        try {
            fetch(`${api().tourPackageWithLocation}?type=CRUISE${urlParams}`)
            .then(rawResult => rawResult.json())
            .then(jsonData => {
                console.log(jsonData, "jsonData")
                setLoading(false)
                setLoadingText("")
                setRJson(groupTourPackageWithLocation(jsonData?.tourDepList))
            });
        } catch (error) {
            console.log('There was an error', error);
        }
    ...
    const onChange=(e, type)=>{
        ...
        console.log("urlParams", urlParams)
        location.href = `/cruise?${urlParams}`
    }
                ...
                <div id="banner" className="mb-5 bg-image d-flex justify-content-center align-items-start" style={{backgroundSize: 'cover', backgroundPosition: 'center'}}>
                    <div id="inner_banner_div" className="position-relative bg-transparent container px-4 d-flex flex-column gap-4">
                        <div className="d-flex align-items-center flex-column">
                            <h1 className="text-white fw-bold" style={{textShadow: '0px 4px 4px rgba(0, 0, 0, 0.25)', fontFamily: '"Montserrat"'}}>CRUISE PACKAGES</h1>
                        </div>
                        ...
                        <div className="d-flex align-items-center flex-column">
                            <button className="custom-button" onClick={()=>location.href="/cruise"}>Clear</button>
                        </div>
                    </div>
                </div>
                            ...
                            return <div className="card d-flex justify-content-start flex-row flex-wrap shadow" style={{borderTopLeftRadius: '2.0rem', borderBottomRightRadius: '1.5rem'}} key={index}>
                                <div className="d-flex justify-content-start col-12 col-xl-5 position-relative">
                                    <img 
                                        ...
                                        alt="Article Image"
                                    />
                                    {value.saleStatus == "best_seller" && <button id="selling_out_button_mobile" type="button" className="d-block d-xl-none btn rounded-3 btn-danger fw-bold m-0 position-absolute bottom-0 end-0" style={{fontFamily: '"Montserrat"', backgroundColor: '#EB5757', width: 'max-content', transform: 'translate(-10%, -35%)', msTransform: 'translate(-10%, -35%)', color:"#EB5757", border:"none", fontWeight:"500"}}>
                                        SELLING OUT
                                    </button>}
                                </div>
                                <div className="p-4 px-xl-5 d-flex justify-content-between flex-column w-100 gap-3" style={{fontFamily: 'Poppins', flex: 1}}>
                                    <div className="d-flex justify-content-between flex-row">
                                        ...
                                        {value.saleStatus !== "best_seller" && <div className="my-n2">
                                            <button id="selling_out_button" type="button" className="d-none d-xl-block btn rounded-3 btn-danger m-0" style={{fontFamily: '"Montserrat"', backgroundColor: '#ffecec', color:"#EB5757", border:"none", fontWeight:"500"}}>
                                                SELLING OUT
                                            </button>
                                        </div>}
                                    </div>
                                ...
                                <hr style={{ color:"EAE9E9" }}/>
                                <div className="flex justify-between">
                                    <button onClick={()=>{location.href=`/cruise/${value.idTourPkg}`}} className="w-fit d-none d-xl-flex justify-content-center rounded-2 border-none fw-bold text-black px-9 py-2" style={{backgroundColor: 'transparent', fontFamily: 'Montserrat', border:"2px solid #2158AA"}}>
                                        VIEW DETAILS
                                    </button>
                                    <div className="fw-bold text-[#D68A2C] text-2xl">
                                        -RM{ value.price }
                                    </div>
                                </div>
                            </div>
                            <button onClick={()=>{location.href=`/cruise/${value.idTourPkg}`}} className="w-fit d-flex d-xl-none justify-content-center border-none fw-bold text-black col-12 px-9 py-2" style={{backgroundColor: 'transparent', fontFamily: 'Montserrat', borderRadius: '0px 0px 20px 0px', border:"2px solid #2158AA"}}>
                                VIEW DETAILS
                            </button>
                            </div>
                        })}
